...
Касательно основы флагов можете почитать в оригинальной статье.
...
В этом пункте важные моменты по флагам, которые могут навредить.
<br> `-XX:+UseFastUnorderedTimeStamps` - Потенциально опасен тем, что заставляет использовать менее строгие метки памяти, не синхронизированные между потоками. При очень маловероятных сценариях способен нарушить задуманную кем-то в своих плагинах/ядрах логику работы `System.nanoTime` и `System.currentTimeMillis`. Сценарий маловероятный, но имеет место быть.
<br> `-XX:+UseStringDeduplication` - Повышает потребление памяти в общем, за счёт дольшего хранения строк в памяти для их реюза, также повышает задержки между сборками мусора, поскольку ему теперь придётся ещё и эти строки прогонять, однако флаг может снизить затраты на пересоздание строк, что в реалиях, когда у всех миллион градиентных префиксов большой выигрыш. Вопрос лишь что важнее - затраты на сборщик или на пересоздания компонентов по миллиарду раз за тик.
<br> `-XX:+OmitStackTraceInFastThrow` - **Никогда не используйте на проде, где вам нужно отслеживать потенциальные проблемы.** Флаг отключит вам вывод полного стактрейса от эксепшнов в консоль, в следствии чего если ВДРУГ у вас возникнет проблема - вы хуй чо поймёте.
